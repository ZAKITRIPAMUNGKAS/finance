<div class="space-y-6">
    ...
    <div class="space-y-4">
        ...
        <div class="bg-white border border-slate-200/80 rounded-3xl p-5 sm:p-6 shadow-sm space-y-4 hover:border-slate-300 transition-all">
                ...
                <div>
                    <div class="flex flex-wrap items-center gap-2">
                        ...
                        <div class="inline-flex items-center gap-1.5 ml-1">
                            <button wire:click="openEditProjectModal({{ $proj->id }})" 
                                    title="Edit Nilai Nominal & Detail Project"
                                    class="px-2.5 py-1 rounded-xl bg-slate-100 hover:bg-slate-200 text-slate-700 hover:text-slate-950 text-[10px] font-extrabold transition-all cursor-pointer flex items-center gap-1 active:scale-95 shadow-2xs">
                                <x-icon name="pencil" class="w-3 h-3 text-slate-600" />
                                <span>Edit Project</span>
                            </button>
                            <button wire:click="deleteProject({{ $proj->id }})" 
                                    wire:confirm="Yakin ingin menghapus project '{{ $proj->name }}' beserta seluruh invoice dan biayanya?"
                                    title="Hapus Project"
                                    class="p-1 rounded-xl hover:bg-rose-50 text-slate-400 hover:text-rose-600 transition-colors cursor-pointer active:scale-95">
            ...
            <div x-data="{ showInvoices: false, showCosts: false }" class="pt-3 border-t border-slate-100 space-y-3">
                <div class="flex flex-col sm:flex-row sm:items-center justify-between gap-3 text-xs">
                    ...
                    <div class="flex flex-wrap items-center gap-2">
                        @if($proj->paid_invoices_total < $proj->total_revenue || $proj->status !== 'completed')
                        <button wire:click="markProjectAsPaid({{ $proj->id }})" 
                            wire:confirm="Tandai project '{{ $proj->name }}' sebagai LUNAS dan catat pemasukan ke rekening utama?"
                            title="Tandai Seluruh Project Lunas"
                            class="px-3.5 py-2 rounded-xl bg-emerald-500 hover:bg-emerald-600 text-white text-xs font-bold transition-all flex items-center justify-center gap-1.5 cursor-pointer active:scale-95 shadow-xs">
                            <x-icon name="check" class="w-3.5 h-3.5" strokeWidth="2.5" />
        ...
        @endforelse
    </div>    <!-- MODAL 1: ADD / EDIT PROJECT -->
    <template x-teleport="body">
        <div x-data="{ 
                open: @entangle('isProjectModalOpen'),
                formatNominal(val) {
                    if (!val) return '';
            ...
            <div @click.outside="$wire.set('isProjectModalOpen', false)" class="relative w-full max-w-lg bg-white border-t sm:border border-slate-200 rounded-t-[32px] sm:rounded-3xl shadow-2xl overflow-hidden max-h-[92vh] flex flex-col animate-in slide-in-from-bottom-6 sm:slide-in-from-bottom-2 duration-200">
                <div class="px-6 py-4 border-b border-slate-100 flex items-center justify-between shrink-0">
                    <h3 class="text-base font-extrabold text-slate-900">{{ $projectId ? 'Edit Project & Nilai Kontrak' : 'Project Freelance Baru' }}</h3>
                ...
                <form wire:submit.prevent="saveProject" class="p-6 pb-8 sm:pb-6 space-y-4 overflow-y-auto">
                    ...
                    <div class="pt-3 border-t border-slate-100 flex items-center justify-end gap-2.5">
                        ...
                        <button type="submit" class="px-5 py-2.5 rounded-xl bg-slate-950 text-[#C6F24D] text-xs font-extrabold hover:bg-slate-800 cursor-pointer shadow-sm">
            ...
            <div @click.outside="$wire.set('isCostModalOpen', false)" class="relative w-full max-w-md bg-white border-t sm:border border-slate-200 rounded-t-[32px] sm:rounded-3xl shadow-2xl overflow-hidden animate-in slide-in-from-bottom-6 sm:slide-in-from-bottom-2 duration-200">
                ...
                <form wire:submit.prevent="saveCost" class="p-6 pb-8 sm:pb-6 space-y-4">
                    ...
                    <div class="grid grid-cols-1 sm:grid-cols-2 gap-3">
                        ...
                        <div>
                            <label class="block text-xs font-bold text-slate-700 mb-1">Potong dari Rekening</label>
                            <select wire:model.defer="cost_account_id" class="w-full bg-[#F8F9FA] border border-slate-200 rounded-2xl px-3.5 py-2.5 text-xs font-semibold text-slate-900 focus:outline-none focus:border-slate-900">
                                <option value="">Hanya Catatan Project</option>
